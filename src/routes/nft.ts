...
router.post("/mint", [
    body("nftCount").trim().escape().notEmpty().isNumeric().withMessage("Invalid nftCount"),
    body("mintTo").trim().escape().notEmpty().withMessage("Invalid mintTo"),
    body("collectionName").trim().escape().notEmpty().withMessage("Invalid collectionName"),
    body("collectionSymbol").trim().escape().notEmpty().withMessage("Invalid collectionSymbol")
], handleInvalidInput, async (req: Request, res: Response) => {

    const { nftCount, mintTo, collectionName, collectionSymbol } = req.body;

    const neededMaxDepth = calculateMaxDepth(nftCount);
    ...
    let mintToPublicKey
    try {
        mintToPublicKey = new PublicKey(mintTo)
    } catch (err) {
        logger.error(err)
        return res.status(HttpResponseCode.BAD_REQUEST).json({
            data: null,
    ...
    if (initBalance < storageCost) {
        ...
        return res.status(HttpResponseCode.INTERNAL_SERVER_ERROR).json({ data: null, errors: [{ message: "Not enough SOL to allocate the merkle tree. Donate some." }] })
    }
